...
|---|---|---|---|---|
| `detach_double_fork` (`server/lifecycle.rs`) | Unix `fork → setsid → fork` daemonization helper; sole `unsafe` block in `dreamd-core` (WEG-99–104) | **Zero call sites** — not invoked by `dreamd watch` or any shipped path | **Keep unused** — `dreamd service install` supervises foreground `dreamd watch` on both backends: the systemd `--user` unit (AILAB-190, `Type=simple`) and the macOS LaunchAgent (AILAB-169, `KeepAlive` + foreground `ProgramArguments`), so neither is this helper's call site; it still has zero call sites and is retained only for a possible non-systemd / non-launchd background path | A non-systemd / non-launchd background mode is scheduled (not AILAB-190 / AILAB-169) |
