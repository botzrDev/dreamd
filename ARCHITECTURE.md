...
|---|---|---|---|---|
| `detach_double_fork` (`server/lifecycle.rs`) | Unix `fork → setsid → fork` daemonization helper; sole `unsafe` block in `dreamd-core` (WEG-99–104) | **Zero call sites** — not invoked by `dreamd watch` or any shipped path | **Keep unused** — `dreamd service install` (AILAB-190) writes a systemd `--user` unit that supervises foreground `dreamd watch` (`Type=simple`), so this helper is not its call site; retained only for a possible non-systemd background path | A non-systemd background mode is scheduled (not AILAB-190) |
