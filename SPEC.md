...
The frontmatter block MUST contain `last_updated` (ISO 8601 UTC timestamp of the cycle run), `prompt_version` (implementation-defined; use `"deterministic-only"` for the network-free fallback), and `cluster_key` (the promoted cluster's full `skill_action` string). A writer MAY additionally emit `citations`: the episodic `id` values the lesson was composed from, space-separated inside a single quoted scalar (`citations: "evt_… evt_…"`). Readers MUST tolerate its absence, and a reader that does not recognize it ignores it like any other unknown field. The `<!-- dreamd:lesson -->` opening tag MUST carry an `id` attribute set to the exemplar event's `id` and a `cluster` attribute set to the cluster's full `skill_action` string. The closing `<!-- /dreamd:lesson -->` tag MUST appear on its own line immediately after the lesson body. Implementations MAY emit multiple lesson blocks (one per promoted cluster) separated by blank lines.
