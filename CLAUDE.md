...
- **`npx dreamd-mcp <cmd>` routes only `init` to the native subcommand — everything else is prefixed with `mcp`.** The shim (`packages/dreamd-mcp/bin/dreamd-mcp.js:108`) is `args[0] === 'init' ? args : ['mcp', ...args]`, so `npx dreamd-mcp watch` runs `dreamd mcp watch` (invalid), **not** `dreamd watch`. Consequence: npm-only users — the primary v0.1 surface — have no one-liner to start the shared `dreamd watch` daemon, which is the safe concurrent-writer config (`dreamd mcp` auto-detects a reachable daemon socket → Phase 2 single serialized writer; else Phase 1 in-process, independent writer per server). The shim caches the native binary at `~/.cache/dreamd-mcp/<version>/dreamd` but it's not on PATH. Tracked Urgent as WEG-280 (3 undecided fixes: shim `watch` passthrough / expose cached-binary path / auto-spawn). Surfaced by DEMO-5 docs, 2026-06-19. → [[npx-shim-routes-only-init]]
